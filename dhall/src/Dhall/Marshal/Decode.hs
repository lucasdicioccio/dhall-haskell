...
instance (Functor f, FromDhall (f (Result f))) => FromDhall (Fix f) where
    autoWith inputNormalizer = Decoder{..}
      where
        ...
        expected =
            (\x -> Pi mempty "result" (Const Core.Type) (Pi mempty "Make" (Pi mempty "_" x "result") "result"))
...
instance {-# OVERLAPPING #-} GenericFromDhall a1 (M1 S s1 (K1 i1 a1) :*: M1 S s2 (K1 i2 a2)) where
    genericAutoWithNormalizer _ _ _ =
        pure $
            Decoder
...
functionWith inputNormalizer (Encoder{..}) (Decoder extractIn expectedIn) =
  ...
  where
    ...
    extractOut e =
        pure
            ( \i -> case extractIn (Core.normalizeWith normalizer_ (App e (embed i))) of
                Success o -> o
          ...
          where
            ...
            err =
                case duplicates of
                    ...
                    ([]) -> "No duplicates" -- technically unreachable
        Failure f -> Failure f
    extractOut expr = typeError expectedOut expr
